...
| The new `bridge-phrases` nested config **does** persist — unlike its `conversation-config` counterpart | Verified 2026-07-29 against the `cli-test` org and the CI org (`TestBridgePhrasesCRUD`, PR #167). A create → update → read-back cycle returned **all** of `config.phrases.messages`, `config.phrases.localized` (`es-US`), `config.tools[]` (per-tool override incl. its phrases), `config.smart_turn_timeout_seconds`, and `config.randomize_bridge_phrases` intact. Each is asserted on typed values in that test, so a future silent drop fails the live gate rather than passing green. **Note the contrast:** the `conversation-config` equivalents of `smart_turn_timeout_seconds` and the unified `messages` list are *still* silently dropped in prod (rows below, [cli#100](https://github.com/asksyllable/syllable-cli/issues/100)) — so on the new surface these fields work, and on the old one they don't. `config.tools[].tool_name` is **not** validated against real tools; an unknown name is stored as-is. |
