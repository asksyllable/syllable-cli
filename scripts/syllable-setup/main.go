...
type Config struct {
	...
	Environments map[string]Env    `yaml:"environments,omitempty" json:"environments"`
	Orgs         map[string]Org    `yaml:"orgs,omitempty" json:"orgs"`
}
...
type Org struct {
	...
	Envs   map[string]OrgEnv `yaml:"envs,omitempty" json:"envs"`
}
...
// sentinel sent to the browser in place of real key values
const keyPlaceholder = "__existing__"
...
func main() {
	...
	if err != nil {
		fatalf("cannot determine config path: %v", err)
	}

	// real holds the actual keys; masked is sent to the browser
	real := loadConfig(cfgPath)
	masked := maskKeys(real)
	...
	http.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		...
		page := strings.ReplaceAll(htmlPage, "%%INIT_CONFIG%%", string(cfgJSON))
		fmt.Fprint(w, page)
	})
	...
	http.HandleFunc("/save", func(w http.ResponseWriter, r *http.Request) {
		...
		// Merge: any sentinel value → restore real key
		merged := mergeKeys(&incoming, real)

		if err := writeConfig(cfgPath, merged); err != nil {
		...
		// Update in-memory real config so subsequent saves are consistent
		real = merged
		masked = maskKeys(real)
...
// maskKeys returns a copy of cfg with all api_key values replaced by
// keyPlaceholder (if non-empty) or "" (if empty).
func maskKeys(cfg *Config) *Config {
	...
	for name, org := range cfg.Orgs {
		masked := Org{
			APIKey: maskKey(org.APIKey),
			Envs:   map[string]OrgEnv{},
		}
		for env, e := range org.Envs {
			masked.Envs[env] = OrgEnv{APIKey: maskKey(e.APIKey)}
		}
...
// mergeKeys replaces any sentinel values in incoming with the real keys from stored.
func mergeKeys(incoming *Config, stored *Config) *Config {
	for name, org := range incoming.Orgs {
		if org.APIKey == keyPlaceholder {
...
func loadConfig(path string) *Config {
	...
	if cfg.Orgs == nil {
		cfg.Orgs = map[string]Org{}
	}
	// ensure each org has an Envs map
	for name, org := range cfg.Orgs {
		if org.Envs == nil {
			org.Envs = map[string]OrgEnv{}
		}
		cfg.Orgs[name] = org
	}
	return cfg
...
// ---------------------------------------------------------------------------
// Embedded UI  (%%INIT_CONFIG%% = JSON-encoded masked config)
// ---------------------------------------------------------------------------
...
// Ensure every org has an envs map
...
function pwField(labelText, storedValue) {
  ...
  // indicator that a key is already set
  if (hasExisting) {
  ...
  // getter: returns EXISTING sentinel if user left it blank and one was already set
...
function collectState() {
  // Walk org cards and collect any modified single-key inputs
  Object.keys(state.orgs).forEach(function(orgName) {
    var org = state.orgs[orgName];
    var hasEnvKeys = org.envs && Object.keys(org.envs).length > 0;
    if (!hasEnvKeys) {
      // single key mode — read from the DOM input
